...
namespace TasksManagement.API.Core
{
    public class GlobalExceptionHandler
    {
        ...
        {
            try //try to execute everything that will happen next, among other things, our useCase
            {
                await _next(httpContext);
            }
            catch (Exception ex)
            {
                httpContext.Response.ContentType = "application/json"; //when exception happens client doesn't know why
                object response = null;
                var statusCode = StatusCodes.Status500InternalServerError;

                switch (ex) //recognizing which exception happend 
                {
